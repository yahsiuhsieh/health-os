...
class AIClientTests(unittest.TestCase):
    def test_openrouter_uses_default_free_model_and_parses_structured_response(self) -> None:
        ...
        self.assertEqual(result.draft.today_actions, ("先保守起步。",))
    ...
    def test_openrouter_error_falls_back_to_rule_based_draft(self) -> None:
        ...
        self.assertGreaterEqual(len(result.draft.today_actions), 1)
    ...
    def test_openrouter_invalid_json_falls_back_to_rule_based_draft(self) -> None:
        ...
        self.assertEqual(result.provider, "rule_based")
        self.assertTrue(result.draft.sleep_insight)
...
def _draft_json(*, summary: str = "恢復資料可用，今天維持穩定節奏。") -> str:
    return json.dumps(
        {
            "summary": summary,
            "sleep_insight": "睡眠接近近期平均。",
            "activity_insight": "昨天活動量穩定。",
            "recovery_insight": "恢復指標維持穩定。",
            "today_actions": ["先保守起步。"],
            "cautions": [],
        },
        ensure_ascii=False,
...
def _payload() -> dict:
    return build_prompt_payload(
        ...
        },
        {
            ...
            "baseline_7d": {"averages": {"steps": 7000, "exercise_minutes": 35}},
            "baseline_28d": {"averages": {"steps": 6800, "exercise_minutes": 32}},
